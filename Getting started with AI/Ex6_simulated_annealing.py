...
def main():
    ...
    for step in range(steps):
        ...
        T = 1.0 - step / steps  # Linear temperature reduction schedule
        ...
        for i in range(tracks):
            ...
            # Calculate the acceptance probability using simulated annealing
            prob = math.exp(-(S_new - S_old) / T)

            # Accept or reject the new solution based on the acceptance probability
            if random.random() < prob:
                x[i], y[i] = x_new, y_new   # Accept the new solution
